...
/** Archive current week to AuditLog and clear bookings. Advances trainingDate. */
export async function resetWeek(nextTrainingDateISO: string): Promise<void> {
  await prisma.$transaction(async (tx) => {
    const bookings = await tx.booking.findMany({ include: { slot: true } });
    await tx.auditLog.create({
      data: {
        action: "resetWeek",
        payload: JSON.stringify({
          ...
          bookings: bookings.map((b) => ({
            ...
            slotDate: b.slot.date.toISOString(),
            name: b.name,
            whatsapp: b.whatsapp,
            email: b.email,
            uber: b.uber,
    ...
    await tx.setting.upsert({
      ...
      create: { key: "trainingDate", value: nextTrainingDateISO },
      update: { value: nextTrainingDateISO },
    });
  });
}
